  ...
  const map = useMapEvents({
    click: async (e) => {
      ...
      const clickedPermanentNode = permanentNodes.find(node => {
        ...
        return clickPoint.distanceTo(markerPoint) < clickRadius;
      });
      ...
      try {
        if (mode === 'node') {
          const query = `[out:json];node(around:20,${lat},${lng});out body;`;
          ...
          const query = `[out:json];way(around:20,${lat},${lng})[highway];(._;>;);out body;`;
          const response = await fetch('https://overpass-api.de/api/interpreter', { method: 'POST', body: query });
          const data = await response.json();
...
const MapComponent: React.FC<MapProps> = ({ 
  ...
  return (
    <Box p={4} bg="gray.100" borderRadius="md" boxShadow="lg" w="full">
      ...
      <Box h="400px" w="100%" borderRadius="md" overflow="hidden">
        <MapContainer center={center} zoom={zoom} style={{ height: '100%', width: '100%' }}>
          ...
          {permanentNodes.map((node) => (
            <Marker key={node.nodeId} position={node.position} icon={blackIcon}>
              <Popup>
                Node ID: {node.nodeId}
              </Popup>
            </Marker>
